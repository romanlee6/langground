...
#     feature_sizes = [5,6,4,4,4,9]
#     def one_hot_encode_instance(feature_vector, feature_sizes):
#         one_hot_encoded = []
#         start_index = 0
#
#         for i, feature_size in enumerate(feature_sizes):
#             one_hot_feature = [0] * feature_size
#             feature_value = feature_vector[i]
#             one_hot_feature[feature_value] = 1
#             one_hot_encoded.extend(one_hot_feature)
#             start_index += feature_size
#
...
#             count = count1 * count2
#
#             sim = cosine_similarity(one_hot_encode_instance(loc1,feature_sizes),one_hot_encode_instance(loc2,feature_sizes))
#             dis = 1- sim
#
#             # else:
#             #     if loc1[2] != loc2[2]:
...
for comm, locs in all_comms_to_full.items():
    for loc, count in locs.items():
        ...
        room, bomb, seq0,seq1,seq2, action = loc

        llm_coms = offline_data[(offline_data['room'] == room) & (offline_data['bomb'] == bomb) & (offline_data['seq0'] == seq0) & (offline_data['seq1'] == seq1) & (offline_data['seq2'] == seq2) & (offline_data['action'] == action)]
