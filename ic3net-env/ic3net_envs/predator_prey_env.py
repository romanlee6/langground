...
class PredatorPreyEnv(gym.Env):
    ...
    def init_args(self, parser):
        ...
        # env.add_argument('--prey_loc_res', action="store_true", default=False,
        #                  help="if restrain the range of prey loc")
        # env.add_argument('--prey_loc_range', type=int, default=3,
        #                  help="prey location range (top x row)")

    def multi_agent_init(self, args):

        # General variables defining the environment : CONFIG
        params = ['dim', 'vision', 'moving_prey', 'mode', 'enemy_comm','prey_loc_res','eval_mode']
        for key in params:
        ...
        self.prey_loc_res = args.prey_loc_res
    ...
    def _get_cordinates(self):
        if self.prey_loc_res:

            # Generate random indices within the specified range
            if self.eval_mode =='eval':
                idx_prey = np.random.choice([0,4,12,17,21], self.prey, replace=False)
            else:
                indx_prey = 0
                while indx_prey in [0,4,12,17,21]:
                    idx_prey = np.random.choice(np.prod(self.dims), self.prey, replace=False)


            # Generate random indices for predators
            idx_predator = np.random.choice(np.prod(self.dims), self.npredator, replace=False)

            # Combine the indices
            idx = np.concatenate((idx_predator, idx_prey))

            # Convert indices to coordinates
            coords = np.vstack(np.unravel_index(idx, self.dims)).T
